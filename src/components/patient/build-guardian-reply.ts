...
export function buildGuardianTurn(
  ...
  if (/שלום|היי|מה\s*נשמע|עזרה|help/i.test(lower)) {
    return {
      reply: `שלום! אני עוזר PHYSIOSHIELD לשאלות על כאב ותרגול (${areaLabel}). שאלו למשל «איך אני מתקדם?» או על רמות כאב. אני בודק את הדיווחים שלכם לפני שאמליץ על שינוי בעומס.`,
      offer: null,
    };
  }
