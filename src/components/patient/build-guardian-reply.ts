...
export function buildGuardianTurn(
  ...
  if (/דגל\s*אדום|red\s*flag|התראה|מסוכן/i.test(q) || (lower.includes('כאב') && /חמור|גבוה|10|תשע|שמונה|7\b/.test(q))) {
    return {
      reply: patient.hasRedFlag
        ...
        : 'דגל אדום מופעל כשמדווחים כאב 6+ או מאמץ 8+ אחרי תרגיל. אם זה קורה — עצרו, דווחו, והמטפל יקבל התראה.',
      offer: null,
    };
  }
  ...
  if (/כאב|כואב|pain/i.test(q)) {
    ...
    return {
      reply:
        'עדיין אין מספיק דיווחי כאב אחרי תרגילים כדי לשרטט מגמה אישית — וזה בסדר. בינתיים: תרגלו לפי ההנחיות בכרטיס, עצרו אם מופיע כאב חד או חד, ואחרי «כבוצע» דווחו כאב ומאמץ כדי שנוכל ללוות אתכם טוב יותר. בכל ספק — פנו למטפל.',
      offer: null,
    };
  }
  ...
  if (/שלום|היי|מה\s*נשמע|עזרה|help/i.test(lower)) {
    return {
      reply: `שלום! אני כאן לשאלות על כאב, תרגול והתקדמות באזור ${areaLabel}. אפשר לשאול «איך אני מתקדם?» או על רמות כאב. כל כ־3 ימים המערכת בודקת את הדיווחים ומכינה הצעות למטפל — שינויים בתוכנית נכנסים רק אחרי אישורו.`,
      offer: null,
    };
  }
  ...
  return {
    reply: appendReasoning(
      `אשמח לעזור — אפשר לשאול על התרגילים, על כאב אחרי אימון, או «איך אני מתקדם?». ` +
        `כרגע בתוכנית: ${exerciseCount} תרגילים, רצף ${patient.currentStreak} ימים` +
        (lastPain != null
          ? `, ודיווח כאב אחרון ${lastPain}/10`
          : '. עדיין אין דיווח כאב אחרון — אחרי תרגול כדאי לדווח כדי שנוכל להתאים הנחיות') +
        `. המערכת גם בודקת את ההתקדמות כל כ־3 ימים ומעבירה הצעות למטפל — שינויים בתוכנית נכנסים רק אחרי אישורו.`
    ),
    offer: null,
  };
}
